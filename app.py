...
SUBSCRIPTION_PLANS = {
    ...
    'elite': {
        ...
        'currency': 'INR',
        'features': ['Premium Summaries', '1000 min/month', 'Priority Processing', 'Premium Support']
    }
}
...
def initialize_new_user(user_id):
    today = datetime.now()
    next_month = today + relativedelta(months=1, day=1, hour=0, minute=0, second=0, microsecond=0)
...
def dashboard():
    ...
    plan_data = {
        ...
        'minutes_limit': SUBSCRIPTION_PLANS[user_data.get('subscription', {}).get('plan', 'free')]['minutes_limit'],
        'next_billing_date': user_data.get('subscription', {}).get('next_billing_date', datetime.now()).strftime('%B %d, %Y'),
        'recent_videos': user_data.get('usage', {}).get('video_history', [])[:5]  # Get last 5 videos
    }
    
    return render_template('dashboard.html', user_data=user_data, plan_data=plan_data, plans=SUBSCRIPTION_PLANS)
...
def pricing():
    ...
    return render_template('pricing.html', plans=SUBSCRIPTION_PLANS, current_plan=current_plan, plan_data=plan_data, is_authenticated=is_authenticated)
...
def summarize_video():
    ...
    try:
        ...
        if not transcript:
            return jsonify({'error': 'Could not extract transcript from this video'}), 400
        ...
        return jsonify({
            ...
            'summary': summary,
            'transcript': transcript[:1000] + '...' if len(transcript) > 1000 else transcript  # Shortened transcript preview
        })
...
def get_user_usage():
    ...
    usage_data = {
        ...
        'percentage_used': round((user_data.get('usage', {}).get('minutes_used_this_month', 0) / 
                              SUBSCRIPTION_PLANS[user_data.get('subscription', {}).get('plan', 'free')]['minutes_limit']) * 100, 1)
    }
...
def verify_payment():
    ...
    try:
        ...
        return jsonify({'success': True, 'message': f'Payment verified. Subscribed to {SUBSCRIPTION_PLANS[plan_id]["name"]}.'})
...
def my_videos():
    ...
    return render_template('my_videos.html', videos=video_history, plan_data=plan_data, plans=SUBSCRIPTION_PLANS)
...
def update_user_usage(user_id, duration_minutes, video_id, title, summary):
    ...
    user_ref.update({
        'usage.minutes_used_this_month': firestore.Increment(round(duration_minutes, 2)),
...
def update_user_subscription(user_id, plan_id, payment_id):
    ...
    next_billing_date = today + relativedelta(months=1, day=1, hour=0, minute=0, second=0, microsecond=0)
